		...
		success:function(data){
			...
			if(data.totalSize>0){
		        ...
		        if(data.currentBlock>1){
		        	htmlList +='<a href="list?blockNum=1&pageNum=1&category=${param.category}">처음</a>';
		        	htmlList +='<a href="list?blockNum=${currentBlock-1}&pageNum=${startPage-1}&category=${param.category}">이전</a>';
		        }
		        for(var i=data.startPage; i<=data.endPage; i++){
		        	if(data.currentPage==i) htmlList +='<font color="red"><b>'+i+'</b></font>';
		        	if(data.currentPage!=i) htmlList +='<a href="list?pageNum=${i}&category=${param.category}">'+i+'</a>';
		        }
		        if(data.currentBlock<data.totalBlock){
		        	htmlList +='<a href="list?blockNum=${currentBlock+1}&pageNum=${endPage+1}&category=${param.category}">다음</a>';
		        	htmlList +='<a href="list?blockNum=${totalBlock}&pageNum=${totalPage}&category=${param.category}">마지막</a>';
		        }
		        htmlList += '<td>';
		        htmlList += '</tr>';
			}
